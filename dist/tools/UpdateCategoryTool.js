...
export async function execute(input, api) {
    try {
        ...
        if (input.categoryGroupId !== undefined)
            categoryData.category_group_id = input.categoryGroupId;
        if (Object.keys(categoryData).length === 0) {
            throw new Error("No fields to update. Provide at least one of: name, note, goalTarget, categoryGroupId.");
        }
        const response = await api.categories.updateCategory(budgetId, input.categoryId, { category: categoryData });
